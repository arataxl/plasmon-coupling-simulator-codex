...
- 標準のCDA/QCMワークフローではAuナノ球1〜20個について、粒径、3D配置、表面間ギャップ、偏光、均一媒質を変え、
...
`experimental/post-submission` ブランチには、既定OFFの **「Experimental: approximate quadrupole coupling」** トグルを追加しています。単一球Mieの電気四極子係数 `a_2` から電気四極子を構成し、近似的な電気双極子–電気四極子結合だけを加える実験機能です。四極子–四極子結合と磁気多重極を含まないため、多粒子結果の定量精度や厳密なエネルギー保存は保証しません。近赤外域の定性的な傾向確認にのみ用い、使用有無はJSON/CSV来歴に記録されます。

同ブランチには、直径2〜500 nmの均一Au**単一球だけ**を対象にした **「単一粒子・厳密Mie理論モード」** もあります。収束まで和を取る全次数の電気・磁気Mie係数を使い、CDA、QCM、実験的四極子近似は使用しません。2〜100 nmでは、既存の単一粒子FCDA近似との比較・検証に意図的に使えます。これは多粒子CDAの2〜100 nm範囲を拡張するものではありません。局所・均一球およびバルク光学定数という材料前提は残ります。
...
UIの入力表は各粒子の径・x/y/z（nm）を最大50個まで編集できる。21〜50個は、全対の表面間
ギャップが5 nm超であるQCMなしの古典CDA探索に限定し、同期APIではなくSSE計算経路を使う。
QCM適用域および1〜5 nmのCDA近似限界警告域は最大20個のままとする。送信前にも全対の表面間
